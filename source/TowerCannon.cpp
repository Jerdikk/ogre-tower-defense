...
TowerCannon::TowerCannon(TowerBase* parent)
  : Tower(parent, "Cannon", OFFENSIVE)
{
  mpShotGraphics = NULL;
  setRange(700.0f);
  setAttackSpeed(0.666f);
  mAtkPower = 50.0;
}

TowerCannon::~TowerCannon(void)
{
  printf("~TowerCannon\n");
  if (mpShotGraphics)
  {
...
void TowerCannon::sell(void)
{
}

bool TowerCannon::createGraphics(void)
{
  bool retval = false;

  if (mpsSceneMgr)
  {
    mMaterialName = "Material/Tower/Cannon";
    ...
    if (retval)
    {
      ...
      mpShotGraphics->begin("Material/Tower/Cannon/Shot1", Ogre::RenderOperation::OT_LINE_LIST);
...
void TowerCannon::update(float t)
{
  static const float sShotFadeOut = 0.1;
